pipeline {
    ...
    stages {
        ...
        // This stage pauses the pipeline and fails if the SonarQube Quality Gate is not 'OK'.
        // ==========================================================
        stage('Quality Gate') {
            steps {
                timeout(time: 5, unit: 'MINUTES') {
                    // abortPipeline: true will automatically fail the build.
            ...
            steps {
                script {
                    ...
                    withAWS(credentials: AWS_CREDENTIALS_ID, region: env.AWS_REGION) {
                        ...
                        // Add '--alias' to create a context name without special characters,
                        // which is a more robust way to handle authentication.
